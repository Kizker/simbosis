...
const form = useForm({
  ...
  permissions: props.user.permissions ? props.user.permissions.map(p => p.name) : [],
  _method: 'put'
});
...
const submit = () => {
  // If user unchecks changePassword, clear password field so it isn't sent
  if (!changePassword.value) {
    form.password = '';
  }
  
  // Note: Since we have file uploads, we MUST use POST with _method spoofing
  form.post(`/harmony-access/users/${props.user.id}`);
};
</script>
  ...
  <AdminLayout title="Edit User">
    <div class="w-full">
      ...
      <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
          <h2 class="text-3xl font-extrabold text-on-surface tracking-tight">Edit User</h2>
          <p class="text-sm text-on-surface-variant mt-1">Perbarui informasi, role, dan permission pengguna ini.</p>
        </div>
      ...
      <div class="liquid-glass dark:liquid-glass-dark border border-outline-variant/30 rounded-xl shadow-sm p-6 sm:p-8">
        <form @submit.prevent="submit" class="grid gap-6 lg:grid-cols-2">
          ...
          <div class="lg:col-span-2">
            ...
            <div class="flex items-center gap-6">
              <div class="shrink-0 w-24 h-24 rounded-full overflow-hidden bg-surface-variant/30 border border-outline-variant/30 flex items-center justify-center">
                <img v-if="avatarPreview || props.user.author_profile?.avatar_path" 
                     :src="avatarPreview || (props.user.author_profile.avatar_path.startsWith('http') ? props.user.author_profile.avatar_path : '/storage/' + props.user.author_profile.avatar_path)" 
          ...
          <!-- Password field (Toggled) -->
          <div class="space-y-4 lg:col-span-2">
            <label class="flex items-center gap-3 p-4 rounded-xl border border-outline-variant/30 bg-surface-variant/10 cursor-pointer hover:bg-surface-variant/20 transition-colors w-max">
              <input
                type="checkbox"
                v-model="changePassword"
                class="rounded border-outline-variant text-primary focus:ring-primary h-5 w-5 bg-white dark:bg-slate-900"
              />
              <div class="text-sm font-bold text-on-surface">Ubah Password?</div>
            </label>

            <div v-show="changePassword" class="space-y-2">
              <label class="block text-sm font-bold text-on-surface mb-2">Password Baru (Opsional)</label>
          ...
          <div class="flex items-center justify-end gap-3 pt-6 mt-2 border-t border-outline-variant/20 lg:col-span-2">
            <button
              ...
              class="px-8 py-3 text-sm font-bold rounded-full btn-simbiosis-save text-white shadow-sm disabled:opacity-50"
            >
              Simpan Perubahan
            </button>
          </div>
